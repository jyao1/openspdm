...
/**
  Test A: receiving a VERSION message with a larger list of available versions than indicated.
...
**/
void TestSpdmRequesterGetVersionCaseA(void **state) {
  RETURN_STATUS        Status;
  SPDM_TEST_CONTEXT    *SpdmTestContext;
  SPDM_DEVICE_CONTEXT  *SpdmContext;
  UINT8                VersionNumberEntryCount;
...
/**
  Test B: receiving a correct VERSION message with available version 10.0 and 10.1, but
  the requester do not have compatible versions with the responder.
  Expected behavior: client returns a Status of RETURN_DEVICE_ERROR.
**/
void TestSpdmRequesterGetVersionCaseB(void **state) {
  RETURN_STATUS        Status;
  SPDM_TEST_CONTEXT    *SpdmTestContext;
  SPDM_DEVICE_CONTEXT  *SpdmContext;
  UINT8                VersionNumberEntryCount;
...
/**
  Test C: receiving a VERSION message in SPDM version 1.1 (in the header), but correct
  1.0-version format, with available version 1.0 and 1.1.
  Expected behavior: client returns a Status of RETURN_DEVICE_ERROR.
**/
void TestSpdmRequesterGetVersionCaseC(void **state) {
  RETURN_STATUS        Status;
  SPDM_TEST_CONTEXT    *SpdmTestContext;
  SPDM_DEVICE_CONTEXT  *SpdmContext;
  UINT8                VersionNumberEntryCount;
...
/**
  Test D: receiving a VERSION message with wrong SPDM RequestResponseCode (in this
  ...
  Expected behavior: client returns a Status of RETURN_DEVICE_ERROR.
**/
void TestSpdmRequesterGetVersionCaseD(void **state) {
  RETURN_STATUS        Status;
  SPDM_TEST_CONTEXT    *SpdmTestContext;
  SPDM_DEVICE_CONTEXT  *SpdmContext;
  UINT8                VersionNumberEntryCount;
...
int SpdmRequesterGetVersionTestMain(void) {
  const struct CMUnitTest SpdmRequesterGetVersionTests[] = {
      ...
      // Successful response
      cmocka_unit_test(TestSpdmRequesterGetVersionCaseA),
      // Successful response + device error
      cmocka_unit_test(TestSpdmRequesterGetVersionCaseB),
      cmocka_unit_test(TestSpdmRequesterGetVersionCaseC),
      cmocka_unit_test(TestSpdmRequesterGetVersionCaseD),
  };
