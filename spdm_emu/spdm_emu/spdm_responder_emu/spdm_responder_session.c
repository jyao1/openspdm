...
spdm_vendor_defined_response_mine_t  m_vendor_defined_response = {
  ...
  sizeof(pci_protocol_header_t) + sizeof(pci_ide_km_query_resp_t), // payload_length
  {
...
**/
return_status
test_spdm_process_packet_callback (
  ...
  )
{
  ...
  if (!is_app_message) {
    ...
    ASSERT ((request_size >= sizeof(spdm_vendor_defined_request_mine_t)) && (request_size < sizeof(spdm_vendor_defined_request_mine_t) + 4));
    ASSERT (spdm_request->header.request_response_code == SPDM_VENDOR_DEFINED_REQUEST);
    ASSERT (spdm_request->standard_id == SPDM_REGISTRY_ID_PCISIG);
    ASSERT (spdm_request->vendor_id == SPDM_VENDOR_ID_PCISIG);
    ASSERT (spdm_request->payload_length == sizeof(pci_protocol_header_t) + sizeof(pci_ide_km_query_t));
    ASSERT (spdm_request->pci_protocol.protocol_id == PCI_PROTOCAL_ID_IDE_KM);
    ASSERT (spdm_request->pci_ide_km_query.header.object_id == PCI_IDE_KM_OBJECT_ID_QUERY);

    copy_mem (response, &m_vendor_defined_response, sizeof(m_vendor_defined_response));
    *response_size = sizeof(m_vendor_defined_response);
  } else {
    app_request = request;
    ASSERT (request_size == sizeof(secure_session_request_mine_t));
    ASSERT (app_request->mctp_header.message_type == MCTP_MESSAGE_TYPE_PLDM);
    ASSERT (app_request->pldm_header.pldm_type == PLDM_MESSAGE_TYPE_CONTROL_DISCOVERY);
    ASSERT (app_request->pldm_header.pldm_command_code == PLDM_CONTROL_DISCOVERY_COMMAND_GET_TID);

    copy_mem (response, &m_secure_session_response, sizeof(m_secure_session_response));
  ...
  )
{
  ...
  status = test_spdm_process_packet_callback (
             session_id,
             is_app_message,
             request,
             request_size,
