...
void test_spdm_crypt_spdm_get_dmtf_subject_alt_name_from_bytes(void **state) {
  ...
  ret = spdm_get_dmtf_subject_alt_name_from_bytes(m_subject_alt_name_buffer1, sizeof (m_subject_alt_name_buffer1), common_name, &common_name_size, dmtf_oid, &dmtf_oid_size);
  ...
  zero_mem (common_name, common_name_size);
  zero_mem (dmtf_oid, dmtf_oid_size);
  ret = spdm_get_dmtf_subject_alt_name_from_bytes(m_subject_alt_name_buffer2, sizeof (m_subject_alt_name_buffer2), common_name, &common_name_size, dmtf_oid, &dmtf_oid_size);
  assert_int_equal((int)ret, RETURN_SUCCESS);
  assert_memory_equal(m_dmtf_oid, dmtf_oid, sizeof (m_dmtf_oid));
  assert_string_equal(common_name, "ACME:WIDGET:1234567890");
  ...
  zero_mem (common_name, common_name_size);
  zero_mem (dmtf_oid, dmtf_oid_size);
  ret = spdm_get_dmtf_subject_alt_name_from_bytes(m_subject_alt_name_buffer3, sizeof (m_subject_alt_name_buffer3), common_name, &common_name_size, dmtf_oid, &dmtf_oid_size);
  assert_int_equal((int)ret, RETURN_SUCCESS);
  assert_memory_equal(m_dmtf_oid, dmtf_oid, sizeof (m_dmtf_oid));
  assert_string_equal(common_name, "ACME:WIDGET:1234567890");
}
...
void test_spdm_crypt_spdm_get_dmtf_subject_alt_name(void **state) {
  ...
  status = read_input_file ("rsa2048/end_requester.cert.der", (void **)&file_buffer, &file_buffer_size);
  assert_true(status);
  dmtf_oid_size = 64;
  common_name_size = 64;
  ret = spdm_get_dmtf_subject_alt_name(file_buffer, file_buffer_size, common_name, &common_name_size, dmtf_oid, &dmtf_oid_size);
  assert_int_equal((int)ret, RETURN_SUCCESS);
  assert_memory_equal(m_dmtf_oid, dmtf_oid, sizeof (m_dmtf_oid));
  assert_string_equal(common_name, "ACME:WIDGET:1234567890");
  free (file_buffer);

  status = read_input_file ("rsa3072/end_requester.cert.der", (void **)&file_buffer, &file_buffer_size);
  assert_true(status);
  dmtf_oid_size = 64;
  common_name_size = 64;
  ret = spdm_get_dmtf_subject_alt_name(file_buffer, file_buffer_size, common_name, &common_name_size, dmtf_oid, &dmtf_oid_size);
  ...
  status = read_input_file ("ecp256/end_requester.cert.der", (void **)&file_buffer, &file_buffer_size);
  assert_true(status);
  dmtf_oid_size = 64;
  common_name_size = 64;
  ret = spdm_get_dmtf_subject_alt_name(file_buffer, file_buffer_size, common_name, &common_name_size, dmtf_oid, &dmtf_oid_size);
  ...
  status = read_input_file ("ecp384/end_requester.cert.der", (void **)&file_buffer, &file_buffer_size);
  assert_true(status);
  dmtf_oid_size = 64;
  common_name_size = 64;
  ret = spdm_get_dmtf_subject_alt_name(file_buffer, file_buffer_size, common_name, &common_name_size, dmtf_oid, &dmtf_oid_size);
...
int spdm_crypt_lib_test_main(void) {
  const struct CMUnitTest spdm_crypt_lib_tests[] = {
      cmocka_unit_test(test_spdm_crypt_spdm_get_dmtf_subject_alt_name_from_bytes),
      cmocka_unit_test(test_spdm_crypt_spdm_get_dmtf_subject_alt_name),
      cmocka_unit_test(test_spdm_crypt_spdm_x509_certificate_check)
  };

  return cmocka_run_group_tests(spdm_crypt_lib_tests, spdm_crypt_lib_setup, spdm_crypt_lib_teardown);
}
