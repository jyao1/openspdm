  ...
  )
{
  ...
  spdm_secured_message_callbacks_t.get_sequence_number = test_get_sequence_number;
  spdm_secured_message_callbacks_t.get_max_random_number_count = test_get_max_random_number_count;
  ...
  if (session_id != NULL) {
    ...
    if (RETURN_ERROR(status)) {
      DEBUG ((DEBUG_ERROR, "spdm_encode_secured_message - %p\n", status));
      return status;
    }
  ...
  )
{
  ...
  if (SecuredMessageSessionId != NULL) {
    *session_id = SecuredMessageSessionId;
    ...
    secured_message_context = spdm_get_secured_message_context_via_session_id (spdm_context, *SecuredMessageSessionId);
    if (secured_message_context == NULL) {
    ...
    if (RETURN_ERROR(status)) {
      DEBUG ((DEBUG_ERROR, "spdm_decode_secured_message - %p\n", status));
      spdm_secured_message_get_last_spdm_error_struct (secured_message_context, &spdm_error);
      spdm_set_last_spdm_error_struct (spdm_context, &spdm_error);
      return RETURN_UNSUPPORTED;
    }
    ...
    if (RETURN_ERROR(status)) {
      ...
      *is_app_message = FALSE;
      if (SecuredMessageSessionId == NULL) {
        ...
        DEBUG ((DEBUG_ERROR, "transport_decode_message - expect encapsulated normal but got session (%08x)\n", *SecuredMessageSessionId));
        return RETURN_UNSUPPORTED;
      }
    }
  } else {
