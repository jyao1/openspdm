  ...
  )
{
  ...
  case 0x2:
  {
    ...
    temp_buf_size = sizeof(spdm_digest_response_t) + spdm_get_hash_size (m_use_hash_algo);
    ...
    spdm_hash_all (m_use_hash_algo, m_local_certificate_chain, MAX_SPDM_MESSAGE_BUFFER_SIZE, &digest[0]);
  ...
  case 0x13:
  {
    ...
    ((spdm_context_t*)spdm_context)->connection_info.algorithm.bash_hash_algo = m_use_hash_algo;
    digest_count = 4;
    temp_buf_size = sizeof(spdm_digest_response_t) + digest_count * spdm_get_hash_size (m_use_hash_algo);
    spdm_response = (void *)temp_buf;
    ...
    for (index = 0; index < digest_count; index++) {
      spdm_hash_all (m_use_hash_algo, m_local_certificate_chain, MAX_SPDM_MESSAGE_BUFFER_SIZE, &digest[index * spdm_get_hash_size (m_use_hash_algo)]);
      spdm_response->header.param2 |= (1 << index);
      if (index == 0) continue;
      digest[(index + 1) * spdm_get_hash_size (m_use_hash_algo) - 1] = 0;
    }
  ...
  case 0x15:
  {
    ...
    ((spdm_context_t*)spdm_context)->connection_info.algorithm.bash_hash_algo = m_use_hash_algo;
    temp_buf_size = sizeof(spdm_message_header_t) + MAX_HASH_SIZE * MAX_SPDM_SLOT_COUNT + 1;
    spdm_response = (void *)temp_buf;
...
void test_spdm_requester_get_digests_case2(void **state) {
  ...
  assert_int_equal (spdm_context->transcript.message_b.buffer_size, sizeof(spdm_get_digest_request_t) + sizeof(spdm_digest_response_t) + spdm_get_hash_size (spdm_context->connection_info.algorithm.bash_hash_algo));
}
...
void test_spdm_requester_get_digests_case11(void **state) {
  ...
  status = spdm_get_digest (spdm_context, &slot_mask, &total_digest_buffer);
  assert_int_equal (status, RETURN_DEVICE_ERROR);
  assert_int_equal (spdm_context->transcript.message_b.buffer_size, sizeof(spdm_get_digest_request_t));
}
...
void test_spdm_requester_get_digests_case15(void **state) {
  ...
  spdm_context->local_context.peer_cert_chain_provision_size = MAX_SPDM_MESSAGE_BUFFER_SIZE;
  set_mem (m_local_certificate_chain, MAX_SPDM_MESSAGE_BUFFER_SIZE, (uint8)(0xFF));
  spdm_context->transcript.message_b.buffer_size = spdm_context->transcript.message_b.max_buffer_size;
...
void test_spdm_requester_get_digests_case16(void **state) {
  ...
  spdm_context->local_context.peer_cert_chain_provision_size = MAX_SPDM_MESSAGE_BUFFER_SIZE;
  set_mem (m_local_certificate_chain, MAX_SPDM_MESSAGE_BUFFER_SIZE, (uint8)(0xFF));
  spdm_context->transcript.message_b.buffer_size = spdm_context->transcript.message_b.max_buffer_size - (sizeof(spdm_digest_response_t));
...
void test_spdm_requester_get_digests_case17(void **state) {
  ...
  status = spdm_get_digest (spdm_context, &slot_mask, &total_digest_buffer);
  assert_int_equal (status, RETURN_SECURITY_VIOLATION);
  assert_int_equal (spdm_context->error_state, SPDM_STATUS_ERROR_CERTIFICATE_FAILURE);
}
...
int spdm_requester_get_digests_test_main(void) {
  ...
  return cmocka_run_group_tests(spdm_requester_get_digests_tests, spdm_unit_test_group_setup, spdm_unit_test_group_teardown);
}
