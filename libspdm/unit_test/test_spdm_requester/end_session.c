  ...
  )
{
  ...
  copy_mem (secured_message_context->application_secret.response_data_encryption_key, key, secured_message_context->aead_key_size);
}
  ...
  )
{
  ...
  copy_mem (secured_message_context->application_secret.response_data_salt, salt, secured_message_context->aead_iv_size);
}
  ...
  )
{
  ...
  case 0x2:
  {
    ...
    spdm_transport_test_encode_message (spdm_context, &session_id, FALSE, FALSE, temp_buf_size, temp_buf, response_size, response);    
...
void test_spdm_requester_end_session_case2(void **state) {
  ...
  assert_int_equal (spdm_secured_message_get_session_state (spdm_context->session_info[0].secured_message_context), SPDM_SESSION_STATE_NOT_STARTED);
  free(data);
}
...
int spdm_requester_end_session_test_main(void) {
  ...
  return cmocka_run_group_tests(spdm_requester_end_session_tests, spdm_unit_test_group_setup, spdm_unit_test_group_teardown);
}
