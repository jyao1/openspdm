  ...
  )
{
  ...
  case 0x2:
  {
      ...
      if (m_local_certificate_chain == NULL) {
        read_responder_public_certificate_chain (m_use_hash_algo, m_use_asym_algo, &m_local_certificate_chain, &m_local_certificate_chain_size, NULL, NULL);
      }
      if (m_local_certificate_chain == NULL) {
        return RETURN_OUT_OF_RESOURCES;
      }
      count = (m_local_certificate_chain_size + MAX_SPDM_CERT_CHAIN_BLOCK_LEN + 1) / MAX_SPDM_CERT_CHAIN_BLOCK_LEN;
      if (calling_index != count - 1) {
        portion_length = MAX_SPDM_CERT_CHAIN_BLOCK_LEN;
        remainder_length = (uint16)(m_local_certificate_chain_size - MAX_SPDM_CERT_CHAIN_BLOCK_LEN * (calling_index + 1));
      } else {
        portion_length = (uint16)(m_local_certificate_chain_size - MAX_SPDM_CERT_CHAIN_BLOCK_LEN * (count - 1));
        remainder_length = 0;
      }
      ...
      copy_mem (spdm_response + 1, (uint8 *)m_local_certificate_chain + MAX_SPDM_CERT_CHAIN_BLOCK_LEN * calling_index, portion_length);
  ...
  case 0xB:
  {
      ...
      if (m_local_certificate_chain == NULL) {
        ...
        cert_buffer = (uint8 *)m_local_certificate_chain + sizeof(spdm_cert_chain_t) + hash_size;
        cert_buffer_size = m_local_certificate_chain_size - sizeof(spdm_cert_chain_t) - hash_size;
        if (!x509_get_cert_from_cert_chain (cert_buffer, cert_buffer_size, -1, &leaf_cert_buffer, &leaf_cert_buffer_size)) {
  ...
  case 0xD:
  {
      ...
      if (m_local_certificate_chain == NULL) {
        read_responder_public_certificate_chain_by_size (m_use_hash_algo, m_use_asym_algo, TEST_CERT_SMALL, &m_local_certificate_chain, &m_local_certificate_chain_size, NULL, NULL);
      }
      if (m_local_certificate_chain == NULL) {
        return RETURN_OUT_OF_RESOURCES;
      }
  ...
  case 0xE:
  {
      ...
      count = (m_local_certificate_chain_size + get_cert_length + 1) / get_cert_length;
      if (calling_index != count - 1) {
        portion_length = get_cert_length;
        remainder_length = (uint16)(m_local_certificate_chain_size - get_cert_length * (calling_index + 1));
      } else {
        portion_length = (uint16)(m_local_certificate_chain_size - get_cert_length * (count - 1));
        remainder_length = 0;
      }
      ...
      spdm_response->portion_length = portion_length;
      spdm_response->remainder_length = remainder_length;
      copy_mem (spdm_response + 1, (uint8 *)m_local_certificate_chain + get_cert_length * calling_index, portion_length);
  ...
  case 0xF:
  {
      ...
      if (m_local_certificate_chain == NULL) {
        read_responder_public_certificate_chain_by_size (m_use_hash_algo, m_use_asym_algo, TEST_CERT_MAXUINT16, &m_local_certificate_chain, &m_local_certificate_chain_size, NULL, NULL);
      }
      if (m_local_certificate_chain == NULL) {
        return RETURN_OUT_OF_RESOURCES;
      }
...
void test_spdm_requester_get_certificate_case2(void **state) {
  ...
  spdm_context->connection_info.capability.flags |= SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_CERT_CAP;
  read_responder_public_certificate_chain (m_use_hash_algo, m_use_asym_algo, &data, &data_size, &hash, &hash_size);
  count = (data_size + MAX_SPDM_CERT_CHAIN_BLOCK_LEN - 1) / MAX_SPDM_CERT_CHAIN_BLOCK_LEN;
  spdm_context->local_context.peer_root_cert_hash_provision_size = hash_size;
  spdm_context->local_context.peer_root_cert_hash_provision = hash;
  spdm_context->local_context.peer_cert_chain_provision = NULL;
  spdm_context->local_context.peer_cert_chain_provision_size = 0;
...
void test_spdm_requester_get_certificate_case13(void **state) {
  ...
  read_responder_public_certificate_chain_by_size (m_use_hash_algo, m_use_asym_algo, TEST_CERT_SMALL, &data, &data_size, &hash, &hash_size);
  spdm_context->local_context.peer_root_cert_hash_provision_size = hash_size;
  spdm_context->local_context.peer_root_cert_hash_provision = hash;
  spdm_context->local_context.peer_cert_chain_provision = NULL;
  spdm_context->local_context.peer_cert_chain_provision_size = 0;
...
void test_spdm_requester_get_certificate_case14(void **state) {
  ...
  cert_chain_size = sizeof(cert_chain);
  zero_mem (cert_chain, sizeof(cert_chain));
  status = spdm_get_certificate_choose_length (spdm_context, 0, get_cert_length, &cert_chain_size, cert_chain);
...
void test_spdm_requester_get_certificate_case15(void **state) {
  ...
  spdm_context->connection_info.capability.flags |= SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_CERT_CAP;
  // Loading Root certificate and saving its hash
  read_responder_public_certificate_chain_by_size (m_use_hash_algo, m_use_asym_algo, TEST_CERT_MAXUINT16, &data, &data_size, &hash, &hash_size);
  spdm_context->local_context.peer_root_cert_hash_provision_size = hash_size;
  spdm_context->local_context.peer_root_cert_hash_provision = hash;
  spdm_context->local_context.peer_cert_chain_provision = NULL;
  spdm_context->local_context.peer_cert_chain_provision_size = 0;
...
int spdm_requester_get_certificate_test_main(void) {
  ...
  return cmocka_run_group_tests(spdm_requester_get_certificate_tests, spdm_unit_test_group_setup, spdm_unit_test_group_teardown);
}
