...
void test_spdm_responder_key_exchange_case1(void **state) {
  ...
  spdm_get_random_number (SPDM_RANDOM_DATA_SIZE, m_spdm_key_exchange_request1.random_data);
  ...
  *(uint16 *)ptr = (uint16)opaque_key_exchange_req_size;
  ...
  status = spdm_get_response_key_exchange (spdm_context, m_spdm_key_exchange_request1_size, &m_spdm_key_exchange_request1, &response_size, response);
...
void test_spdm_responder_key_exchange_case2(void **state) {
  ...
  spdm_get_random_number (SPDM_RANDOM_DATA_SIZE, m_spdm_key_exchange_request2.random_data);
  ...
  *(uint16 *)ptr = (uint16)opaque_key_exchange_req_size;
  ...
  ptr += opaque_key_exchange_req_size;
  response_size = sizeof(response);
  status = spdm_get_response_key_exchange (spdm_context, m_spdm_key_exchange_request2_size, &m_spdm_key_exchange_request2, &response_size, response);
  assert_int_equal (status, RETURN_SUCCESS);
...
int spdm_responder_key_exchange_test_main(void) {
  ...
  return cmocka_run_group_tests(spdm_responder_key_exchange_tests, spdm_unit_test_group_setup, spdm_unit_test_group_teardown);
}
