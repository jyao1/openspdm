...
void test_spdm_responder_version_case1(void **state) {
  ...
  status = spdm_get_response_version (spdm_context, m_spdm_get_version_request1_size, &m_spdm_get_version_request1, &response_size, response);
  assert_int_equal (status, RETURN_SUCCESS);
  assert_int_equal (response_size, sizeof(spdm_version_response) + DEFAULT_SPDM_VERSION_ENTRY_COUNT * sizeof(spdm_version_number_t));
...
void test_spdm_responder_version_case2(void **state) {
  ...
  response_size = sizeof(response);
  status = spdm_get_response_version (spdm_context, m_spdm_get_version_request2_size, &m_spdm_get_version_request2, &response_size, response);
  assert_int_equal (status, RETURN_SUCCESS);
...
void test_spdm_responder_version_case6(void **state) {
  ...
  response_size = sizeof(response);
  status = spdm_get_response_version (spdm_context, m_spdm_get_version_request3_size, &m_spdm_get_version_request3, &response_size, response);
  assert_int_equal (status, RETURN_SUCCESS);
  assert_int_equal (response_size, sizeof(spdm_error_response_t));
  spdm_response = (void *)response;
  assert_int_equal (spdm_response->header.request_response_code, SPDM_ERROR);
...
int spdm_responder_version_test_main(void) {
  ...
  return cmocka_run_group_tests(spdm_responder_version_tests, spdm_unit_test_group_setup, spdm_unit_test_group_teardown);
}
