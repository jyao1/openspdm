...
void test_spdm_responder_heartbeat_case1(void **state) {
  ...
  status = spdm_get_response_heartbeat (spdm_context, m_spdm_heartbeat_request1_size, &m_spdm_heartbeat_request1, &response_size, response);
...
void test_spdm_responder_heartbeat_case2(void **state) {
  ...
  response_size = sizeof(response);
  status = spdm_get_response_heartbeat (spdm_context, m_spdm_heartbeat_request2_size, &m_spdm_heartbeat_request2, &response_size, response);
  assert_int_equal (status, RETURN_SUCCESS);
...
int spdm_responder_heartbeat_test_main(void) {
  ...
  return cmocka_run_group_tests(spdm_responder_heartbeat_tests, spdm_unit_test_group_setup, spdm_unit_test_group_teardown);
}
