...
void test_spdm_responder_digests_case1(void **state) {
  ...
  status = spdm_get_response_digests (spdm_context, m_spdm_get_digests_request1_size, &m_spdm_get_digests_request1, &response_size, response);
  assert_int_equal (status, RETURN_SUCCESS);
  assert_int_equal (response_size, sizeof(spdm_digest_response_t) + spdm_get_hash_size(spdm_context->connection_info.algorithm.bash_hash_algo));
...
void test_spdm_responder_digests_case2(void **state) {
  ...
  response_size = sizeof(response);
  status = spdm_get_response_digests (spdm_context, m_spdm_get_digests_request2_size, &m_spdm_get_digests_request2, &response_size, response);
  assert_int_equal (status, RETURN_SUCCESS);
...
void test_spdm_responder_digests_case8(void **state) {
  ...
  response_size = sizeof(response);
  spdm_context->transcript.message_b.buffer_size = spdm_context->transcript.message_b.max_buffer_size - sizeof(spdm_get_digest_request_t);
  status = spdm_get_response_digests (spdm_context, m_spdm_get_digests_request1_size, &m_spdm_get_digests_request1, &response_size, response);
  assert_int_equal (status, RETURN_SUCCESS);
  assert_int_equal (response_size, sizeof(spdm_error_response_t));
  spdm_response = (void *)response;
...
int spdm_responder_digests_test_main(void) {
  ...
  return cmocka_run_group_tests(spdm_responder_digests_tests, spdm_unit_test_group_setup, spdm_unit_test_group_teardown);
}
