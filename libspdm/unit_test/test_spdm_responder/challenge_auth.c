...
void test_spdm_responder_challenge_auth_case2(void **state) {
  ...
  response_size = sizeof(response);
  spdm_get_random_number (SPDM_NONCE_SIZE, m_spdm_challenge_request2.nonce);
  status = spdm_get_response_challenge_auth (spdm_context, m_spdm_challenge_request2_size, &m_spdm_challenge_request2, &response_size, response);
  assert_int_equal (status, RETURN_SUCCESS);
...
int spdm_responder_challenge_auth_test_main(void) {
  ...
  return cmocka_run_group_tests(spdm_responder_challenge_auth_tests, spdm_unit_test_group_setup, spdm_unit_test_group_teardown);
}
