...
GLOBAL_REMOVE_IF_UNREFERENCED const uint8 m_sha3_256_digest[SHA3_256_DIGEST_SIZE] = {
  0x3a, 0x98, 0x5d, 0xa7, 0x4f, 0xe2, 0x25, 0xb2, 0x04, 0x5c, 0x17, 0x2d, 0x6b, 0xd3, 0x90, 0xbd,
  0x85, 0x5f, 0x08, 0x6e, 0x3e, 0x9d, 0x52, 0x5b, 0x46, 0xbf, 0xe2, 0x45, 0x11, 0x43, 0x15, 0x32
};

GLOBAL_REMOVE_IF_UNREFERENCED const uint8 m_sha3_384_digest[SHA3_384_DIGEST_SIZE] = {
...
GLOBAL_REMOVE_IF_UNREFERENCED const uint8 m_sha3_512_digest[SHA3_512_DIGEST_SIZE] = {
...
GLOBAL_REMOVE_IF_UNREFERENCED const uint8 m_shake_256_digest[SHAKE256_DIGEST_SIZE] = {
  0x66, 0xc7, 0xf0, 0xf4, 0x62, 0xee, 0xed, 0xd9, 0xd1, 0xf2, 0xd4, 0x6b, 0xdc, 0x10, 0xe4, 0xe2,
  0x41, 0x67, 0xc4, 0x87, 0x5c, 0xf2, 0xf7, 0xa2, 0x29, 0x7d, 0xa0, 0x2b, 0x8f, 0x4b, 0xa8, 0xe0
};

GLOBAL_REMOVE_IF_UNREFERENCED const uint8 m_sm3_256_digest[SHAKE256_DIGEST_SIZE] = {
  0x66, 0xc7, 0xf0, 0xf4, 0x62, 0xee, 0xed, 0xd9, 0xd1, 0xf2, 0xd4, 0x6b, 0xdc, 0x10, 0xe4, 0xe2,
  0x41, 0x67, 0xc4, 0x87, 0x5c, 0xf2, 0xf7, 0xa2, 0x29, 0x7d, 0xa0, 0x2b, 0x8f, 0x4b, 0xa8, 0xe0
};
  ...
  )
{
  ...
  if (status) {
    my_print ("Check value... ");
    if (compare_mem (digest, m_sha3_256_digest, SHA3_256_DIGEST_SIZE) == 0) {
      status = TRUE;
    } else {
      status = FALSE;
    }
  ...
  if (status) {
    my_print ("Check value... ");
    if (compare_mem (digest, m_sha3_384_digest, SHA3_384_DIGEST_SIZE) == 0) {
      status = TRUE;
    } else {
      status = FALSE;
    }
  ...
  if (status) {
    my_print ("Check value... ");
    if (compare_mem (digest, m_sha3_512_digest, SHA3_512_DIGEST_SIZE) == 0) {
      status = TRUE;
    } else {
      status = FALSE;
    }
  ...
  if (status) {
    my_print ("Check value... ");
    if (compare_mem (digest, m_shake_256_digest, SHAKE256_DIGEST_SIZE) == 0) {
      status = TRUE;
    } else {
      status = FALSE;
    }
  ...
  if (status) {
    if (compare_mem (digest, m_sm3_256_digest, SM3_256_DIGEST_SIZE) == 0) {
      status = TRUE;
    } else {
      status = FALSE;
    }
