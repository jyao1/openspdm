  ...
  )
{
  ...
  status = sm2_compute_key (Sm2_1, public2, public2_length, key1, &key1_length);
  if (!status) {
    my_print ("[Fail]");
    sm2_free (Sm2_1);
    sm2_free (Sm2_2);
  ...
  status = sm2_compute_key (Sm2_2, public1, public1_length, key2, &key2_length);
  if (!status) {
    my_print ("[Fail]");
    sm2_free (Sm2_1);
    sm2_free (Sm2_2);
  ...
  my_print ("\n- SM2 Signing ... ");
  status  = sm2_ecdsa_sign (Sm2_1, CRYPTO_NID_SM3_256, message, sizeof(message), signature, &sig_size);
  if (!status) {
    my_print ("[Fail]");
    sm2_free (Sm2_1);
    goto Exit;
  ...
  status = sm2_ecdsa_verify (Sm2_1, CRYPTO_NID_SM3_256, message, sizeof(message), signature, sig_size);
  if (!status) {
    my_print ("[Fail]");
    sm2_free (Sm2_1);
    goto Exit;
  ...
  status = sm2_ecdsa_verify (Sm2_2, CRYPTO_NID_SM3_256, message, sizeof(message), signature, sig_size);
  if (!status) {
    my_print ("[Fail]");
    sm2_free (Sm2_1);
    sm2_free (Sm2_2);
