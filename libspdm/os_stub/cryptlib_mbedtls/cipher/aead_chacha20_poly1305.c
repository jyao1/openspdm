  ...
  )
{
  ...
  ret = mbedtls_chachapoly_encrypt_and_tag (&ctx, (uint32)data_in_size,
                                            iv, a_data, (uint32)a_data_size, data_in, data_out, tag_out);
  ...
  )
{
  ...
  ret = mbedtls_chachapoly_auth_decrypt (&ctx, (uint32)data_in_size,
                                         iv, a_data, (uint32)a_data_size, tag, data_in, data_out);
  mbedtls_chachapoly_free (&ctx);
  if (ret != 0) {
    return FALSE;
  }
