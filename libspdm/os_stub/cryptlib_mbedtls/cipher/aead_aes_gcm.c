  ...
  )
{
  ...
  ret = mbedtls_gcm_setkey (&ctx, MBEDTLS_CIPHER_ID_AES, key, (uint32)(key_size * 8));
  if (ret != 0) {
    return FALSE;
  }

  ret = mbedtls_gcm_crypt_and_tag (&ctx, MBEDTLS_GCM_ENCRYPT, (uint32)data_in_size,
                                   iv, (uint32)iv_size, a_data, (uint32)a_data_size, data_in, data_out,
                                   tag_size, tag_out);
  ...
  )
{
  ...
  ret = mbedtls_gcm_auth_decrypt (&ctx, (uint32)data_in_size,
                                  iv, (uint32)iv_size, a_data, (uint32)a_data_size,
                                  tag, (uint32)tag_size, data_in, data_out);
  mbedtls_gcm_free (&ctx);
  if (ret != 0) {
    return FALSE;
  }
