  ...
  )
{
  ...
  *bn_size = 0;
  ...
  switch (key_tag) {
  case RSA_KEY_N:
    ret = mbedtls_rsa_export(rsa_key, &value, NULL, NULL, NULL, NULL);
    break;
  case RSA_KEY_E:
    ret = mbedtls_rsa_export(rsa_key, NULL, NULL, NULL, NULL, &value);
    break;
  case RSA_KEY_D:
    ret = mbedtls_rsa_export(rsa_key, NULL, NULL, NULL, &value, NULL);
    break;
  case RSA_KEY_Q:
    ret = mbedtls_rsa_export(rsa_key, NULL, NULL, &value, NULL, NULL);
    break;
  case RSA_KEY_P:
    ret = mbedtls_rsa_export(rsa_key, NULL, &value, NULL, NULL, NULL);
    break;
  case RSA_KEY_DP:
  case RSA_KEY_DQ:
  case RSA_KEY_Q_INV:
  ...
  )
{
  ...
  // Check input parameters.
  //
  if (rsa_context == NULL || modulus_length > INT_MAX || public_exponent_size > INT_MAX) {
    return FALSE;
  }
  ...
  if(public_exponent == NULL) {
    ...
    // TBD
    ret = mbedtls_mpi_read_binary(&e, public_exponent, public_exponent_size);
    pe = 0x10001;
  }
