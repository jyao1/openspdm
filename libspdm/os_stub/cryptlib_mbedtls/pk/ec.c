  ...
  )
{
  ...
  ret = mbedtls_mpi_read_binary (&ctx->Q.Y, public_key + half_size, half_size);
  if (ret != 0) {
    return FALSE;
  }
  ...
  )
{
  ...
  *public_key_size = half_size * 2;
  ...
  ret = mbedtls_mpi_write_binary (&ctx->Q.X, &public_key[0 + half_size - x_size], x_size);
  if (ret != 0) {
    return FALSE;
  }
  ret = mbedtls_mpi_write_binary (&ctx->Q.Y, &public_key[half_size + half_size - y_size], y_size);
  if (ret != 0) {
    return FALSE;
  }
  ...
  )
{
  ...
  *public_size = half_size * 2;
  ...
  ret = mbedtls_mpi_write_binary (&ctx->Q.X, &public[0 + half_size - x_size], x_size);
  if (ret != 0) {
    return FALSE;
  }
  ret = mbedtls_mpi_write_binary (&ctx->Q.Y, &public[half_size + half_size - y_size], y_size);
  if (ret != 0) {
    return FALSE;
  }
  ...
  )
{
  ...
  if (ec_context == NULL || peer_public == NULL || key_size == NULL || key == NULL) {
    return FALSE;
  }
  ...
  ret = mbedtls_mpi_read_binary (&ctx->Qp.Y, peer_public + half_size, half_size);
  if (ret != 0) {
    return FALSE;
  }
  ...
  )
{
  ...
  *sig_size = half_size * 2;
  ...
  ret = mbedtls_ecdsa_sign (&ctx->grp, &bn_r, &bn_s, &ctx->d,
                            message_hash, hash_size, myrand, NULL );
  if (ret != 0) {
    return FALSE;
  }
  ...
  ret = mbedtls_mpi_write_binary (&bn_r, &signature[0 + half_size - r_size], r_size);
  if (ret != 0) {
    mbedtls_mpi_free (&bn_r);
    mbedtls_mpi_free (&bn_s);
    return FALSE;
  }
  ret = mbedtls_mpi_write_binary (&bn_s, &signature[half_size + half_size - s_size], s_size);
  if (ret != 0) {
    mbedtls_mpi_free (&bn_r);
    mbedtls_mpi_free (&bn_s);
    return FALSE;
  ...
  )
{
  ...
  ret = mbedtls_ecdsa_verify (&ctx->grp, message_hash, hash_size,
                              &ctx->Q, &bn_r, &bn_s);
  mbedtls_mpi_free (&bn_r);
  mbedtls_mpi_free (&bn_s);
