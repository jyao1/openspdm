...
static const unsigned char m_ffehde2048_p[] = MBEDTLS_DHM_RFC7919_FFDHE2048_P_BIN;
static const unsigned char m_ffehde3072_p[] = MBEDTLS_DHM_RFC7919_FFDHE3072_P_BIN;
static const unsigned char m_ffehde4096_p[] = MBEDTLS_DHM_RFC7919_FFDHE4096_P_BIN;
static const unsigned char m_ffehde2048_g[] = MBEDTLS_DHM_RFC7919_FFDHE2048_G_BIN;
static const unsigned char m_ffehde3072_g[] = MBEDTLS_DHM_RFC7919_FFDHE3072_G_BIN;
static const unsigned char m_ffehde4096_g[] = MBEDTLS_DHM_RFC7919_FFDHE4096_G_BIN;
  ...
  )
{
  ...
  case CRYPTO_NID_FFDHE2048:
    ret = mbedtls_mpi_read_binary (&ctx->P, m_ffehde2048_p, sizeof(m_ffehde2048_p));
    if (ret != 0) {
      goto error;
    }
    ret = mbedtls_mpi_read_binary (&ctx->G, m_ffehde2048_g, sizeof(m_ffehde2048_g));
    if (ret != 0) {
      goto error;
    }
    break;
  case CRYPTO_NID_FFDHE3072:
    ret = mbedtls_mpi_read_binary (&ctx->P, m_ffehde3072_p, sizeof(m_ffehde3072_p));
    if (ret != 0) {
      goto error;
    }
    ret = mbedtls_mpi_read_binary (&ctx->G, m_ffehde3072_g, sizeof(m_ffehde3072_g));
    if (ret != 0) {
      goto error;
    }
    break;
  case CRYPTO_NID_FFDHE4096:
    ret = mbedtls_mpi_read_binary (&ctx->P, m_ffehde4096_p, sizeof(m_ffehde4096_p));
    if (ret != 0) {
      goto error;
    }
    ret = mbedtls_mpi_read_binary (&ctx->G, m_ffehde4096_g, sizeof(m_ffehde4096_g));
    if (ret != 0) {
      goto error;
    }
    break;
  ...
  )
{
  ...
  *public_key_size = final_pub_key_size;
  ...
  ret = mbedtls_dhm_make_public (dh_context, (uint32)*public_key_size, public_key, (uint32)*public_key_size, myrand, NULL);
  if (ret != 0) {
    return FALSE;
  }
  ...
  )
{
  ...
  // Check input parameters.
  //
  if (dh_context == NULL || peer_public_key == NULL || key_size == NULL || key == NULL) {
    return FALSE;
  }
  ...
  ret = mbedtls_dhm_read_public (dh_context, peer_public_key, peer_public_key_size);
  if (ret != 0) {
    return FALSE;
  }

  ret = mbedtls_dhm_calc_secret (dh_context, key, *key_size, key_size, myrand, NULL);
  if (ret != 0) {
    return FALSE;
  }
