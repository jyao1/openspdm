  ...
  )
{
  ...
  if (HMAC_Init_ex ((HMAC_CTX *)hmac_md_ctx, key, (uint32) key_size, Md, NULL) != 1) {
    return FALSE;
  }
  ...
  )
{
  ...
  if (HMAC_CTX_copy ((HMAC_CTX *)new_hmac_md_ctx, (HMAC_CTX *)hmac_md_ctx) != 1) {
    return FALSE;
  }
...
{
  return hmac_md_all (EVP_sha256(), data, data_size, key, key_size, hmac_value);
}
...
{
  return hmac_md_all (EVP_sha384(), data, data_size, key, key_size, hmac_value);
}
...
{
  return hmac_md_all (EVP_sha512(), data, data_size, key, key_size, hmac_value);
}
