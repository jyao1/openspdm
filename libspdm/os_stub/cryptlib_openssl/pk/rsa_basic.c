  ...
  )
{
  ...
  RSA_get0_factors (rsa_key, (const BIGNUM **)&bn_p, (const BIGNUM **)&bn_q);
  RSA_get0_crt_params (rsa_key, (const BIGNUM **)&bn_dp, (const BIGNUM **)&bn_dq, (const BIGNUM **)&bn_q_inv);
  ...
  case RSA_KEY_D:
    ...
    if (RSA_set0_key (rsa_key, BN_dup(bn_n), BN_dup(bn_e), BN_dup(bn_d)) == 0) {
      return FALSE;
    }
  ...
  case RSA_KEY_Q:
    ...
    if (RSA_set0_factors (rsa_key, BN_dup(bn_p), BN_dup(bn_q)) == 0) {
      return FALSE;
    }
  ...
  case RSA_KEY_Q_INV:
    ...
    case RSA_KEY_DQ:
      ...
      break;
    case RSA_KEY_Q_INV:
      bn_q_inv = BN_bin2bn (big_number, (uint32)bn_size, bn_q_inv);
      break;
    default:
      return FALSE;
    }
    if (RSA_set0_crt_params (rsa_key, BN_dup(bn_dp), BN_dup(bn_dq), BN_dup(bn_q_inv)) == 0) {
      return FALSE;
    }
