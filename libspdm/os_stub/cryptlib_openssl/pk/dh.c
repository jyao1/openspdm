  ...
  )
{
  ...
  ret_val = (boolean) DH_generate_parameters_ex (dh_context, (uint32) prime_length, (uint32) generator, NULL);
  if (!ret_val) {
    return FALSE;
  }
  ...
  )
{
  ...
  //
  dh  = (DH *)dh_context;
  bn_p = BN_bin2bn ((const unsigned char *)prime, (int)(prime_length / 8), NULL);
  bn_g = BN_bin2bn ((const unsigned char *)&generator, 1, NULL);
  if ((bn_p == NULL) || (bn_g == NULL) || !DH_set0_pqg (dh, bn_p, NULL, bn_g)) {
    goto error;
  }
  ...
  *public_key_size = final_pub_key_size;
  ...
  if (ret_val) {
    ...
    if (public_key != NULL) {
      ...
      BN_bn2bin (dh_pub_key, &public_key[0 + final_pub_key_size - size]);
    }
  }
