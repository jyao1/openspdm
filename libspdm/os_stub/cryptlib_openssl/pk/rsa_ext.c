  ...
  )
{
  ...
  *bn_size = 0;
  ...
  //
  case RSA_KEY_DP:
    RSA_get0_crt_params (rsa_key, (const BIGNUM **)&bn_key, NULL, NULL);
    break;
  ...
  //
  case RSA_KEY_DQ:
    RSA_get0_crt_params (rsa_key, NULL, (const BIGNUM **)&bn_key, NULL);
    break;
  ...
  //
  case RSA_KEY_Q_INV:
    RSA_get0_crt_params (rsa_key, NULL, NULL, (const BIGNUM **)&bn_key);
    break;
  ...
  )
{
  ...
  if (public_exponent == NULL) {
    ...
    if (BN_bin2bn (public_exponent, (uint32) public_exponent_size, bn_e) == NULL) {
      goto done;
    }
  }

  if (RSA_generate_key_ex ((RSA *) rsa_context, (uint32) modulus_length, bn_e, NULL) == 1) {
   ret_val = TRUE;
  }
