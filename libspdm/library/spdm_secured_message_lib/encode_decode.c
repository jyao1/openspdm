  ...
  )
{
  ...
  ASSERT ((session_type == SPDM_SESSION_TYPE_MAC_ONLY) || (session_type == SPDM_SESSION_TYPE_ENC_MAC));
  ...
  case SPDM_SESSION_STATE_HANDSHAKING:
    if (is_requester) {
      copy_mem (key, secured_message_context->handshake_secret.request_handshake_encryption_key, secured_message_context->aead_key_size);
      copy_mem (salt, secured_message_context->handshake_secret.request_handshake_salt, secured_message_context->aead_iv_size);
      sequence_number = secured_message_context->handshake_secret.request_handshake_sequence_number;
    } else {
      copy_mem (key, secured_message_context->handshake_secret.response_handshake_encryption_key, secured_message_context->aead_key_size);
      copy_mem (salt, secured_message_context->handshake_secret.response_handshake_salt, secured_message_context->aead_iv_size);
      sequence_number = secured_message_context->handshake_secret.response_handshake_sequence_number;
    }
    break;
  case SPDM_SESSION_STATE_ESTABLISHED:
    if (is_requester) {
      copy_mem (key, secured_message_context->application_secret.request_data_encryption_key, secured_message_context->aead_key_size);
      copy_mem (salt, secured_message_context->application_secret.request_data_salt, secured_message_context->aead_iv_size);
      sequence_number = secured_message_context->application_secret.request_data_sequence_number;
    } else {
      copy_mem (key, secured_message_context->application_secret.response_data_encryption_key, secured_message_context->aead_key_size);
      copy_mem (salt, secured_message_context->application_secret.response_data_salt, secured_message_context->aead_iv_size);
      sequence_number = secured_message_context->application_secret.response_data_sequence_number;
    }
    break;
  default:
    ASSERT(FALSE);
  ...
  *(uint64 *)salt = *(uint64 *)salt ^ sequence_number;
  ...
  sequence_num_in_header_size = spdm_secured_message_callbacks_t->get_sequence_number (sequence_number, (uint8 *)&sequence_num_in_header);
  ...
  case SPDM_SESSION_STATE_HANDSHAKING:
    if (is_requester) {
      secured_message_context->handshake_secret.request_handshake_sequence_number = sequence_number;
    } else {
      secured_message_context->handshake_secret.response_handshake_sequence_number = sequence_number;
    }
    break;
  case SPDM_SESSION_STATE_ESTABLISHED:
    if (is_requester) {
      secured_message_context->application_secret.request_data_sequence_number = sequence_number;
    } else {
      secured_message_context->application_secret.response_data_sequence_number = sequence_number;
    }
    break;
  }

  record_header_size = sizeof(spdm_secured_message_a_data_header1_t) + sequence_num_in_header_size + sizeof(spdm_secured_message_a_data_header2_t);

  switch (session_type) {
  case SPDM_SESSION_TYPE_ENC_MAC:
    max_rand_count = spdm_secured_message_callbacks_t->get_max_random_number_count ();
    if (max_rand_count != 0) {
      rand_count = 0;
    ...
    *secured_message_size = total_secured_message_size;
    ...
    record_header2 = (void *)((uint8 *)record_header1 + sizeof(spdm_secured_message_a_data_header1_t) + sequence_num_in_header_size);
    record_header1->session_id = session_id;
    copy_mem (record_header1 + 1, &sequence_num_in_header, sequence_num_in_header_size);
    record_header2->length = (uint16)(cipher_text_size + aead_tag_size);
    enc_msg_header = (void *)(record_header2 + 1);
    enc_msg_header->application_data_length = (uint16)app_message_size;
    copy_mem (enc_msg_header + 1, app_message, app_message_size);
    random_bytes ((uint8 *)enc_msg_header + sizeof(spdm_secured_message_cipher_header_t) + app_message_size, rand_count);
    zero_mem ((uint8 *)enc_msg_header + plain_text_size, aead_pad_size);

    a_data = (uint8 *)record_header1;
    enc_msg = (uint8 *)enc_msg_header;
    dec_msg = (uint8 *)enc_msg_header;
    tag = (uint8 *)record_header1 + record_header_size + cipher_text_size;
  ...
  case SPDM_SESSION_TYPE_MAC_ONLY:
    ...
    *secured_message_size = total_secured_message_size;
    ...
    record_header1->session_id = session_id;
    copy_mem (record_header1 + 1, &sequence_num_in_header, sequence_num_in_header_size);
    record_header2->length = (uint16)(app_message_size + aead_tag_size);
    copy_mem (record_header2 + 1, app_message, app_message_size);
    a_data = (uint8 *)record_header1;
    tag = (uint8 *)record_header1 + record_header_size + app_message_size;
    ...
    result = spdm_aead_encryption (
              ...
              aead_iv_size,
              (uint8 *)a_data,
              record_header_size + app_message_size,
              NULL,
              0,
              tag,
              aead_tag_size,
  ...
  )
{
  ...
  *(uint64 *)salt = *(uint64 *)salt ^ sequence_number;
  ...
  case SPDM_SESSION_TYPE_ENC_MAC:
    ...
    if (compare_mem (record_header1 + 1, &sequence_num_in_header, sequence_num_in_header_size) != 0) {
      spdm_secured_message_set_last_spdm_error_struct (spdm_secured_message_context, &spdm_error);
      return RETURN_SECURITY_VIOLATION;
    }
  ...
  case SPDM_SESSION_TYPE_MAC_ONLY:
    ...
    a_data = (uint8 *)record_header1;
    tag = (uint8 *)record_header1 + record_header_size + record_header2->length - aead_tag_size;
    result = spdm_aead_decryption (
              secured_message_context->aead_cipher_suite,
              key,
              aead_key_size,
