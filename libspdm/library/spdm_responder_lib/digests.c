  ...
  )
{
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, FALSE, 0, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_CERT_CAP)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST, SPDM_GET_DIGESTS, response_size, response);
  ...
  ASSERT (*response_size >= sizeof(spdm_digest_response_t) + hash_size * spdm_context->local_context.slot_count);
  *response_size = sizeof(spdm_digest_response_t) + hash_size * spdm_context->local_context.slot_count;
  ...
  for (index = 0; index < spdm_context->local_context.slot_count; index++) {
    spdm_response->header.param2 |= (1 << index);
    spdm_generate_cert_chain_hash (spdm_context, index, &digest[hash_size * index]);
  }
  //
  // Cache
  //
  ...
  spdm_set_connection_state (spdm_context, SPDM_CONNECTION_STATE_AFTER_DIGESTS);
