  ...
  )
{
  ...
  if (spdm_context->response_state == SPDM_RESPONSE_STATE_NEED_RESYNC) {
    return spdm_responder_handle_response_state(spdm_context, spdm_request->request_response_code, response_size, response);
  }
  ...
  get_response_func = spdm_get_response_func_via_request_code(spdm_request->param1);
  if (get_response_func == NULL) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST, spdm_request->param1, response_size, response);
    return RETURN_SUCCESS;
  }
  status = get_response_func (spdm_context, spdm_context->cache_spdm_request_size, spdm_context->cache_spdm_request, response_size, response);
