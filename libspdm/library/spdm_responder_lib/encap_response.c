...
typedef
return_status
(*spdm_process_encap_response_func) (
  IN     spdm_context_t  *spdm_context,
...
spdm_encap_response_struct_t m_encap_response_struct[] = {
  {SPDM_GET_DIGESTS,     spdm_get_encap_request_get_digest,      spdm_process_encap_response_digest},
  {SPDM_GET_CERTIFICATE, spdm_get_encap_request_get_certificate, spdm_process_encap_response_certificate},
  {SPDM_CHALLENGE,       spdm_get_encap_request_challenge,      spdm_process_encap_response_challenge_auth},
  {SPDM_KEY_UPDATE,      spdm_get_encap_request_key_update,      spdm_process_encap_response_key_update},
};

spdm_encap_response_struct_t *
spdm_get_encap_struct_via_op_code (
  IN     spdm_context_t  *spdm_context,
  ...
  )
{
  ...
  for (index = 0; index < ARRAY_SIZE(m_encap_response_struct); index++) {
    if (m_encap_response_struct[index].request_op_code == request_op_code) {
...
void
spdm_encap_move_to_next_op_code (
  IN     spdm_context_t  *spdm_context
  )
{
  uint8  index;

  ASSERT (spdm_context->encap_context.request_op_code_count <= MAX_ENCAP_REQUEST_OP_CODE_SEQUENCE_COUNT);
  if (spdm_context->encap_context.current_request_op_code == 0) {
    spdm_context->encap_context.current_request_op_code = spdm_context->encap_context.request_op_code_sequence[0];
    return ;
  }
  for (index = 0; index < spdm_context->encap_context.request_op_code_count; index ++) {
    if (spdm_context->encap_context.current_request_op_code == spdm_context->encap_context.request_op_code_sequence[index]) {
      spdm_context->encap_context.current_request_op_code = spdm_context->encap_context.request_op_code_sequence[index + 1];
      return ;
    }
  }
  ASSERT (FALSE);
...
**/
return_status
spdm_process_encapsulated_response (
  IN     spdm_context_t  *spdm_context,
  IN     uintn                encap_response_size,
  IN     void                 *encap_response,
  IN OUT uintn                *encap_request_size,
  ...
  )
{
  ...
  if (spdm_context->encap_context.current_request_op_code != 0) {
    encap_response_struct = spdm_get_encap_struct_via_op_code (spdm_context, spdm_context->encap_context.current_request_op_code);
    ...
    status = encap_response_struct->process_encap_response (spdm_context, encap_response_size, encap_response, &need_continue);
  ...
  status = encap_response_struct->get_encap_request (spdm_context, encap_request_size, encap_request);
  return status;
}
  ...
  )
{
  ...
  zero_mem (&spdm_context->encap_context.last_encap_request_header, sizeof(spdm_context->encap_context.last_encap_request_header));
  ...
  //
  zero_mem (spdm_context->encap_context.request_op_code_sequence, sizeof(spdm_context->encap_context.request_op_code_sequence));
  // Session Mutual Auth
  if (spdm_is_capabilities_flag_supported(spdm_context, FALSE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_PUB_KEY_ID_CAP, 0) ||
  ...
  )
{
  ...
  if (spdm_is_capabilities_flag_supported(spdm_context, FALSE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_PUB_KEY_ID_CAP, 0)) {
    ...
    spdm_context->encap_context.request_op_code_count = 2;
    spdm_context->encap_context.request_op_code_sequence[0] = SPDM_GET_DIGESTS;
    spdm_context->encap_context.request_op_code_sequence[1] = SPDM_CHALLENGE;
  }
}
...
**/
void
spdm_init_key_update_encap_state (
  IN     void                 *context
  )
{
  ...
  )
{
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, FALSE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_ENCAP_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_ENCAP_CAP)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST, SPDM_GET_ENCAPSULATED_REQUEST, response_size, response);
    return RETURN_SUCCESS;
  }
  if (spdm_context->response_state != SPDM_RESPONSE_STATE_PROCESSING_ENCAP) {
  ...
  status = spdm_process_encapsulated_response (context, 0, NULL, &encap_request_size, encap_request);
  if (RETURN_ERROR(status)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_RESPONSE_CODE, 0, response_size, response);
  ...
  )
{
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, FALSE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_ENCAP_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_ENCAP_CAP)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST, SPDM_DELIVER_ENCAPSULATED_RESPONSE, response_size, response);
    return RETURN_SUCCESS;
  }
  if (spdm_context->response_state != SPDM_RESPONSE_STATE_PROCESSING_ENCAP) {
    if (spdm_context->response_state == SPDM_RESPONSE_STATE_NORMAL) {
  ...
  ASSERT (*response_size > sizeof(spdm_encapsulated_response_ack_response_t));
  zero_mem (response, *response_size);
  ...
  encap_request_size = *response_size - sizeof(spdm_encapsulated_response_ack_response_t);
  encap_request = spdm_response + 1;
  ...
  status = spdm_process_encapsulated_response (context, encap_response_size, encap_response, &encap_request_size, encap_request);
  if (RETURN_ERROR(status)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_RESPONSE_CODE, 0, response_size, response);
    spdm_context->response_state = SPDM_RESPONSE_STATE_NORMAL;
    return RETURN_SUCCESS;
  ...
  *response_size = sizeof(spdm_encapsulated_response_ack_response_t) + encap_request_size;
  spdm_response->header.param1 = spdm_context->encap_context.request_id;
  if (encap_request_size == 0) {
    ...
    if (spdm_context->encap_context.req_slot_id != 0) {
      spdm_response->header.param2 = SPDM_ENCAPSULATED_RESPONSE_ACK_RESPONSE_PAYLOAD_TYPE_REQ_SLOT_NUMBER;
      *response_size = sizeof(spdm_encapsulated_response_ack_response_t) + 1;
      *(uint8 *)(spdm_response + 1) = spdm_context->encap_context.req_slot_id;
    }
    spdm_context->response_state = SPDM_RESPONSE_STATE_NORMAL;
  }
