  ...
  )
{
  ...
  case SPDM_RESPONSE_STATE_BUSY:
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_BUSY, 0, response_size, response);
    // NOTE: Need to reset status to Normal in up level
    return RETURN_SUCCESS;
  case SPDM_RESPONSE_STATE_NEED_RESYNC:
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_REQUEST_RESYNCH, 0, response_size, response);
  ...
  case SPDM_RESPONSE_STATE_NOT_READY:
    ...
    spdm_generate_extended_error_response (spdm_context, SPDM_ERROR_CODE_RESPONSE_NOT_READY, 0, sizeof(spdm_error_data_response_not_ready_t), (uint8*)(void*)&spdm_context->error_data, response_size, response);
    // NOTE: Need to reset status to Normal in up level
    return RETURN_SUCCESS;
  case SPDM_RESPONSE_STATE_PROCESSING_ENCAP:
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_REQUEST_IN_FLIGHT, 0, response_size, response);
