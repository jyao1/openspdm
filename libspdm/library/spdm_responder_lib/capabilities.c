...
**/
boolean
spdm_check_request_flag_compability (
  IN uint32 capabilities_flag,
  IN uint8  version
  )
{
  ...
  uint8 handshake_in_the_clear_cap = (uint8)(capabilities_flag>>15)&0x01;
  ...
  )
{
  ...
  if (spdm_context->connection_info.connection_state != SPDM_CONNECTION_STATE_AFTER_VERSION) {
  ...
  if (!spdm_check_request_flag_compability(spdm_request->flags,spdm_request->header.spdm_version)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
  }
  ...
  // Cache
  //
  append_managed_buffer (&spdm_context->transcript.message_a, spdm_request, spdm_request_size);
  ...
  *response_size = sizeof(spdm_capabilities_response);
  ...
  // Cache
  //
  append_managed_buffer (&spdm_context->transcript.message_a, spdm_response, *response_size);
  ...
  spdm_set_connection_state (spdm_context, SPDM_CONNECTION_STATE_AFTER_CAPABILITIES);
