  ...
  )
{
  ...
  status = spdm_append_message_f (session_info, request, request_size - hmac_size);
  ...
  *response_size = sizeof(spdm_psk_finish_response_t);
  ...
  result = spdm_verify_psk_finish_req_hmac (spdm_context, session_info, (uint8 *)request + sizeof(spdm_psk_finish_request_t), hmac_size);
  if (!result) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_DECRYPT_ERROR, 0, response_size, response);
    return RETURN_SUCCESS;
  }
  status = spdm_append_message_f (session_info, (uint8 *)request + request_size - hmac_size, hmac_size);
  if (RETURN_ERROR(status)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
  }

  status = spdm_append_message_f (session_info, spdm_response, *response_size);
  if (RETURN_ERROR(status)) {
    spdm_generate_error_response (spdm_context, SPDM_ERROR_CODE_INVALID_REQUEST, 0, response_size, response);
    return RETURN_SUCCESS;
  }
