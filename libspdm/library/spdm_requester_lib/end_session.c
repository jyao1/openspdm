...
**/
return_status
try_spdm_send_receive_end_session (
  ...
  )
{
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, TRUE, 0, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_CACHE_CAP)) {
  ...
  if (spdm_response.header.request_response_code == SPDM_ERROR) {
    status = spdm_handle_error_response_main(spdm_context, &session_id, NULL, 0, &spdm_response_size, &spdm_response, SPDM_END_SESSION, SPDM_END_SESSION_ACK, sizeof(spdm_end_session_response_mine_t));
    if (RETURN_ERROR(status)) {
      return status;
    }
  ...
  spdm_secured_message_set_session_state (session_info->secured_message_context, SPDM_SESSION_STATE_NOT_STARTED);
  ...
  )
{
  ...
  do {
    status = try_spdm_send_receive_end_session(spdm_context, session_id, end_session_attributes);
