  ...
  )
{
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, TRUE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_KEY_UPD_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_KEY_UPD_CAP)) {
  ...
  spdm_get_random_number (sizeof(spdm_request.header.param2), &spdm_request.header.param2);

  // Create new key
  if ((action & SPDM_KEY_UPDATE_ACTION_RESPONDER) != 0) {
    DEBUG ((DEBUG_INFO, "spdm_create_update_session_data_key[%x] Responder\n", session_id));
    spdm_create_update_session_data_key (session_info->secured_message_context, SPDM_KEY_UPDATE_ACTION_RESPONDER);
  }

  status = spdm_send_spdm_request (spdm_context, &session_id, sizeof(spdm_request), &spdm_request);
  ...
  if (RETURN_ERROR(status) ||
    ...
    if ((action & SPDM_KEY_UPDATE_ACTION_RESPONDER) != 0) {
      DEBUG ((DEBUG_INFO, "spdm_activate_update_session_data_key[%x] Responder old\n", session_id));
      spdm_activate_update_session_data_key (session_info->secured_message_context, SPDM_KEY_UPDATE_ACTION_RESPONDER, FALSE);
    }
    return RETURN_DEVICE_ERROR;
  }

  if ((action & SPDM_KEY_UPDATE_ACTION_RESPONDER) != 0) {
    DEBUG ((DEBUG_INFO, "spdm_activate_update_session_data_key[%x] Responder new\n", session_id, SPDM_KEY_UPDATE_ACTION_RESPONDER));
    spdm_activate_update_session_data_key (session_info->secured_message_context, SPDM_KEY_UPDATE_ACTION_RESPONDER, TRUE);
  }

  DEBUG ((DEBUG_INFO, "spdm_create_update_session_data_key[%x] Requester\n", session_id));
  spdm_create_update_session_data_key (session_info->secured_message_context, SPDM_KEY_UPDATE_ACTION_REQUESTER);
  DEBUG ((DEBUG_INFO, "spdm_activate_update_session_data_key[%x] Requester new\n", session_id));
  spdm_activate_update_session_data_key (session_info->secured_message_context, SPDM_KEY_UPDATE_ACTION_REQUESTER, TRUE);
