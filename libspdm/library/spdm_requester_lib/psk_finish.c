...
**/
return_status
try_spdm_send_receive_psk_finish (
  ...
  )
{
  ...
  status = spdm_append_message_f (session_info, (uint8 *)&spdm_request, spdm_request_size - hmac_size);
  if (RETURN_ERROR(status)) {
    return RETURN_SECURITY_VIOLATION;
  }

  spdm_generate_psk_exchange_req_hmac (spdm_context, session_info, spdm_request.verify_data);

  status = spdm_append_message_f (session_info, (uint8 *)&spdm_request + spdm_request_size - hmac_size, hmac_size);
  if (RETURN_ERROR(status)) {
    return RETURN_SECURITY_VIOLATION;
  }
  ...
  if (spdm_response.header.request_response_code == SPDM_ERROR) {
    status = spdm_handle_error_response_main(spdm_context, &session_id, &session_info->session_transcript.message_f, spdm_request_size, &spdm_response_size, &spdm_response, SPDM_PSK_FINISH, SPDM_PSK_FINISH_RSP, sizeof(spdm_psk_finish_response_max_t));
    if (RETURN_ERROR(status)) {
      return status;
    }
  } else if (spdm_response.header.request_response_code != SPDM_PSK_FINISH_RSP) {
    return RETURN_DEVICE_ERROR;
  }
  ...
  status = spdm_append_message_f (session_info, &spdm_response, spdm_response_size);
  if (RETURN_ERROR(status)) {
    return RETURN_SECURITY_VIOLATION;
  }
  ...
  )
{
  ...
  do {
    status = try_spdm_send_receive_psk_finish(spdm_context, session_id);
