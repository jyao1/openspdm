...
**/
return_status
spdm_requester_respond_if_ready (
  ...
  )
{
  ...
  *response_size = expected_response_size;
  ...
  status = spdm_receive_spdm_response (spdm_context, session_id, response_size, response);
  if (RETURN_ERROR(status)) {
    return RETURN_DEVICE_ERROR;
  }
...
**/
return_status
spdm_handle_response_not_ready (
  IN     spdm_context_t  *spdm_context,
  IN     uint32               *session_id,
  IN OUT uintn                *response_size,
     OUT void                 *response,
  ...
  )
{
  ...
  ASSERT(spdm_response->header.param1 == SPDM_ERROR_CODE_RESPONSE_NOT_READY);
  ASSERT(*response_size == sizeof(spdm_error_response_t) + sizeof(spdm_error_data_response_not_ready_t));
  ...
  return spdm_requester_respond_if_ready(spdm_context, session_id, response_size, response, expected_response_code, expected_response_size);
}
  ...
  )
{
  ...
  if (spdm_response->param1 != SPDM_ERROR_CODE_RESPONSE_NOT_READY) {
    ...
    return spdm_handle_simple_error_response(spdm_context, spdm_response->param1);
  } else {
    return spdm_handle_response_not_ready(spdm_context, session_id, response_size, response, original_request_code, expected_response_code, expected_response_size);
  }
}
