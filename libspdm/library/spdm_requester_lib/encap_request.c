...
**/
void
spdm_register_get_encap_response_func (
  IN  void                          *context,
  IN  spdm_get_encap_response_func  get_encap_response_func
  )
{
...
SpdmGetEncapResponseFuncViaRequestCode (
  IN     uint8                   request_response_code
  )
{
  uintn                index;

  for (index = 0; index < sizeof(m_spdm_get_encap_response_struct)/sizeof(m_spdm_get_encap_response_struct[0]); index++) {
    if (request_response_code == m_spdm_get_encap_response_struct[index].request_response_code) {
      return m_spdm_get_encap_response_struct[index].get_encap_response_func;
    }
  }
  return NULL;
}
  ...
  )
{
  ...
  if (encap_request_size < sizeof(spdm_message_header_t)) {
    spdm_generate_encap_error_response (spdm_context, SPDM_ERROR_CODE_UNSUPPORTED_REQUEST, spdm_requester->request_response_code, encap_response_size, encap_response);
  }

  get_encap_response_func = SpdmGetEncapResponseFuncViaRequestCode (spdm_requester->request_response_code);
  if (get_encap_response_func == NULL) {
    get_encap_response_func = (spdm_get_encap_response_func)spdm_context->get_encap_response_func;
  }
  if (get_encap_response_func != NULL) {
    status = get_encap_response_func (spdm_context, encap_request_size, encap_request, encap_response_size, encap_response);
  } else {
  ...
  )
{
  ...
  spdm_get_encapsulated_request_request_t       *spdm_get_encapsulated_request_request;
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, TRUE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_ENCAP_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_ENCAP_CAP)) {
  ...
  if (mut_auth_requested == SPDM_KEY_EXCHANGE_RESPONSE_MUT_AUTH_REQUESTED_WITH_GET_DIGESTS) {
    ...
    spdm_get_encapsulated_request_request->header.spdm_version = SPDM_MESSAGE_VERSION_11;
    spdm_get_encapsulated_request_request->header.request_response_code = SPDM_GET_ENCAPSULATED_REQUEST;
    ...
    status = spdm_send_spdm_request (spdm_context, session_id, spdm_request_size, spdm_get_encapsulated_request_request);
    ...
    status = spdm_receive_spdm_response (spdm_context, session_id, &spdm_response_size, spdm_encapsulated_request_response);
    if (RETURN_ERROR(status)) {
      return RETURN_DEVICE_ERROR;
    }
    if (spdm_encapsulated_request_response->header.request_response_code != SPDM_ENCAPSULATED_REQUEST) {
      return RETURN_DEVICE_ERROR;
    }
    ...
    encapsulated_request_size = spdm_response_size - sizeof(spdm_encapsulated_request_response_t);
  }
  ...
  while (TRUE) {
    ...
    spdm_deliver_encapsulated_response_request->header.spdm_version = SPDM_MESSAGE_VERSION_11;
    spdm_deliver_encapsulated_response_request->header.request_response_code = SPDM_DELIVER_ENCAPSULATED_RESPONSE;
    ...
    encapsulated_response_size = sizeof(request) - sizeof(spdm_deliver_encapsulated_response_request_t);

    status = SpdmProcessEncapsulatedRequest (spdm_context, encapsulated_request_size, encapsulated_request, &encapsulated_response_size, encapsulated_response);
    if (RETURN_ERROR(status)) {
      return RETURN_DEVICE_ERROR;
    }

    spdm_request_size = sizeof(spdm_deliver_encapsulated_response_request_t) + encapsulated_response_size;
    status = spdm_send_spdm_request (spdm_context, session_id, spdm_request_size, spdm_deliver_encapsulated_response_request);
    if (RETURN_ERROR(status)) {
      return RETURN_DEVICE_ERROR;
    }
    ...
    spdm_response_size = sizeof(response);
    zero_mem (&response, sizeof(response));
    status = spdm_receive_spdm_response (spdm_context, session_id, &spdm_response_size, spdm_encapsulated_response_ack_response);
    if (RETURN_ERROR(status)) {
      return RETURN_DEVICE_ERROR;
    }
    if (spdm_encapsulated_response_ack_response->header.request_response_code != SPDM_ENCAPSULATED_RESPONSE_ACK) {
      return RETURN_DEVICE_ERROR;
    }
    if (spdm_response_size < sizeof(spdm_encapsulated_response_ack_response_t)) {
      return RETURN_DEVICE_ERROR;
    }
    ...
    case SPDM_ENCAPSULATED_RESPONSE_ACK_RESPONSE_PAYLOAD_TYPE_REQ_SLOT_NUMBER:
      if (spdm_response_size >= sizeof(spdm_encapsulated_response_ack_response_t) + sizeof(uint8)) {
        if ((req_slot_id_param != NULL) && (*req_slot_id_param == 0)) {
          *req_slot_id_param = *(uint8 *)(spdm_encapsulated_response_ack_response + 1);
          if (*req_slot_id_param >= spdm_context->local_context.slot_count) {
            return RETURN_DEVICE_ERROR;
          }
        }
        return RETURN_SUCCESS;
    ...
    encapsulated_request_size = spdm_response_size - sizeof(spdm_encapsulated_response_ack_response_t);
  }
...
**/
return_status
spdm_send_receive_encap_request (
