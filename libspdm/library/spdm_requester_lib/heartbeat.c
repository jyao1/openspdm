  ...
  )
{
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, TRUE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_HBEAT_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_HBEAT_CAP)) {
  ...
  if (spdm_response.header.request_response_code == SPDM_ERROR) {
    status = spdm_handle_error_response_main(spdm_context, &session_id, NULL, 0, &spdm_response_size, &spdm_response, SPDM_HEARTBEAT, SPDM_HEARTBEAT_ACK, sizeof(spdm_heartbeat_response_mine_t));
    if (RETURN_ERROR(status)) {
      return status;
    }
