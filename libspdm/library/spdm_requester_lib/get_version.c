  ...
  )
{
  ...
  spdm_version_number_t                       compatible_version_number_entry[MAX_SPDM_VERSION_COUNT];
  ...
  status = spdm_append_message_a (spdm_context, &spdm_request, sizeof(spdm_request));
  if (RETURN_ERROR(status)) {
  ...
  if (spdm_response.header.request_response_code == SPDM_ERROR) {
    shrink_managed_buffer(&spdm_context->transcript.message_a, sizeof(spdm_request));
  ...
  if (spdm_response_size < sizeof(spdm_version_response) + spdm_response.version_number_entry_count * sizeof(spdm_version_number_t)) {
    return RETURN_DEVICE_ERROR;
  }
  spdm_response_size = sizeof(spdm_version_response) + spdm_response.version_number_entry_count * sizeof(spdm_version_number_t);
  //
  // Cache data
  //
  ...
  zero_mem (&compatible_version_number_entry, sizeof(compatible_version_number_entry));
  for (index = 0; index < spdm_response.version_number_entry_count; index++) {
    version = (uint8)((spdm_response.version_number_entry[index].major_version << 4) |
                                                         spdm_response.version_number_entry[index].minor_version);

    if (version == SPDM_MESSAGE_VERSION_11 || version == SPDM_MESSAGE_VERSION_10) {
      compatible_version_number_entry[compatible_version_count] = spdm_response.version_number_entry[index];
  ...
  spdm_context->connection_info.version.spdm_version_count = compatible_version_count;
  copy_mem (spdm_context->connection_info.version.spdm_version, compatible_version_number_entry, sizeof(spdm_version_number_t) * compatible_version_count);
