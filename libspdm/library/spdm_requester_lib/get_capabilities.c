...
**/
boolean
spdm_check_response_flag_compability (
  ...
  )
{
  ...
  if (spdm_response.header.request_response_code == SPDM_ERROR) {
    shrink_managed_buffer(&spdm_context->transcript.message_a, spdm_request_size);
  ...
  if (spdm_request.header.spdm_version != spdm_response.header.spdm_version) {
    return RETURN_DEVICE_ERROR;
  }
  spdm_response_size = sizeof(spdm_capabilities_response);

  if(!spdm_check_response_flag_compability(spdm_response.flags,spdm_response.header.spdm_version)){
    return RETURN_DEVICE_ERROR;
  }
