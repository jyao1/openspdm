  ...
  )
{
  ...
  if (spdm_response.header.request_response_code == SPDM_ERROR) {
    status = spdm_handle_error_response_main(spdm_context, NULL, &spdm_context->transcript.message_b, sizeof(spdm_request), &spdm_response_size, &spdm_response, SPDM_GET_DIGESTS, SPDM_DIGESTS, sizeof(spdm_digests_response_max_t));
    if (RETURN_ERROR(status)) {
      return status;
    }
  ...
  digest_size = spdm_get_hash_size (spdm_context->connection_info.algorithm.bash_hash_algo);
  if (slot_mask != NULL) {
  ...
  for (index = 0; index < digest_count; index++) {
    ...
    internal_dump_data (&spdm_response.digest[digest_size * index], digest_size);
    DEBUG((DEBUG_INFO, "\n"));
  }

  result = spdm_verify_peer_digests (spdm_context, spdm_response.digest, spdm_response_size - sizeof(spdm_digest_response_t));
  if (!result) {
  ...
  if (total_digest_buffer != NULL) {
    copy_mem (total_digest_buffer, spdm_response.digest, digest_size * digest_count);
  }
  ...
  )
{
  ...
  do {
    status = try_spdm_get_digest(spdm_context, slot_mask, total_digest_buffer);
