  ...
  )
{
  ...
  if (!use_psk) {
    status = spdm_send_receive_key_exchange (spdm_context, measurement_hash_type, slot_id, session_id, heartbeat_period, &req_slot_id_param, measurement_hash);
    if (RETURN_ERROR(status)) {
    ...
    status = spdm_send_receive_psk_exchange (spdm_context, measurement_hash_type, session_id, heartbeat_period, measurement_hash);
    if (RETURN_ERROR(status)) {
  ...
  )
{
  ...
  status = spdm_send_receive_end_session (spdm_context, session_id, end_session_attributes);
  ...
  )
{
  ...
  status = spdm_send_request (spdm_context, session_id, is_app_message, request_size, request);
  if (RETURN_ERROR(status)) {
    return RETURN_DEVICE_ERROR;
  }

  status = spdm_receive_response (spdm_context, session_id, is_app_message, response_size, response);
  if (RETURN_ERROR(status)) {
    return RETURN_DEVICE_ERROR;
  }
