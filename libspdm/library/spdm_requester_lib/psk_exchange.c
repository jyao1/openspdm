...
**/
return_status
try_spdm_send_receive_psk_exchange (
  ...
  )
{
  ...
  if (!spdm_is_capabilities_flag_supported(spdm_context, TRUE, SPDM_GET_CAPABILITIES_REQUEST_FLAGS_PSK_CAP, SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_PSK_CAP)) {
  ...
  spdm_request.psk_hint_length = (uint16)spdm_context->local_context.psk_hint_size;
  ...
  DEBUG((DEBUG_INFO, "ClientRandomData (0x%x) - ", spdm_request.context_length));
  internal_dump_data (ptr, spdm_request.context_length);
  DEBUG((DEBUG_INFO, "\n"));
  ptr += spdm_request.context_length;

  status = spdm_build_opaque_data_supported_version_data (spdm_context, &opaque_psk_exchange_req_size, ptr);
  ...
  if (spdm_response.header.request_response_code == SPDM_ERROR) {
    status = spdm_handle_error_response_main(spdm_context, NULL, NULL, 0, &spdm_response_size, &spdm_response, SPDM_PSK_EXCHANGE, SPDM_PSK_EXCHANGE_RSP, sizeof(spdm_psk_exchange_response_max_t));
    if (RETURN_ERROR(status)) {
      return status;
    }
  ...
  *session_id = (req_session_id << 16) | rsp_session_id;
  ...
  ptr = (uint8 *)&spdm_response + sizeof(spdm_psk_exchange_response_t) + measurement_summary_hash_size + spdm_response.context_length;
  status = spdm_process_opaque_data_version_selection_data (spdm_context, spdm_response.opaque_length, ptr);
  if (RETURN_ERROR(status)) {
    spdm_free_session_id (spdm_context, *session_id);
    return RETURN_UNSUPPORTED;
  }
  ...
  DEBUG((DEBUG_INFO, "ServerRandomData (0x%x) - ", spdm_response.context_length));
  ...
  status = spdm_append_message_k (session_info, &spdm_response, spdm_response_size - hmac_size);
  if (RETURN_ERROR(status)) {
    spdm_free_session_id (spdm_context, *session_id);
    return RETURN_SECURITY_VIOLATION;
  }
  ...
  result = spdm_verify_psk_exchange_rsp_hmac (spdm_context, session_info, verify_data, hmac_size);
  if (!result) {
    spdm_free_session_id (spdm_context, *session_id);
  ...
  )
{
  ...
  do {
    status = try_spdm_send_receive_psk_exchange(spdm_context, measurement_hash_type, session_id, heartbeat_period, measurement_hash);
